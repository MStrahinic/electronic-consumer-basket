...
public class Narucivanje extends HttpServlet{
    ...
    public void izvrsi(HttpServletRequest req, HttpServletResponse res){
        try{
            ...
            if(korpa.getStavke().size()>0){
                KorisnikBean kb=(KorisnikBean)session.getAttribute("korisnik");
                Kontroler.getInstance().naruciProizvode(kb);
            }else{
                greska="Greska kod narucivanja! Vasa korpa je prazna! ";
     ...
     public void pozoviJSP(HttpServletRequest req, HttpServletResponse res){
        try {
            rd = sc.getRequestDispatcher("/NaruceneKorpe");
            rd.forward(req, res);
        }
        catch (Exception e){
